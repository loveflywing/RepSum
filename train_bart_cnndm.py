...
def parse_args():
    ...
    parser.add_argument(
        "--pt_data", type=str, default='data_cache/cnndm/train_val.pt', help="A pt file to load the training and eval data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="如果设置了，则模型会进行训练，否则不会",
    )
    parser.add_argument(
        "--do_val",
        type=bool,
        default=False,
        help="设置的话模型会进行验证，否则不会",
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default='/storage/longjing/bart-large',
    ...
    parser.add_argument("--data_process_dir", type=str, default="./data_cache/cnndm/train_val.pt",
                        help="Where to store the processed data pt file.")
    parser.add_argument("--output_dir", type=str, default='./tst-summarization_cnndm', help="Where to store the final model.")
    parser.add_argument("--eval_size", type=int, default=5000, help="验证集的大小，总数不得超过14000")
    parser.add_argument("--save_step_nums", type=int, default=5000, help="多少步骤保存一次模型")
